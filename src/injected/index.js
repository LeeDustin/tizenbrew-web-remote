...
(function startWebRemoteBridge() {
  ...
  function createOverlay() {
    ...
    root.querySelector('.sub').textContent = 'Remote recovery: Red or ↑ ↑ ↓ ↓ OK · Back hides';
  ...
  function connectSocket() {
    ...
    nextSocket.addEventListener('open', () => {
      if (!pageActive || socket !== nextSocket) {
        try { nextSocket.close(); } catch { /* The page is already leaving. */ }
        return;
      }
      clearTimeout(socketTimer);
      transport = 'WebSocket';
  ...
  function suspendBridge() {
    ...
    socket = null;
    if (previousSocket) {
      try { previousSocket.close(1000, 'Page navigation'); } catch { /* The document is already unloading. */ }
    }
  }
  ...
  try {
    ...
    // TizenBrew normally registers package.json keys; runtime registration is a fallback.
  }
  window.addEventListener('keydown', pairingShortcut, true);
